...
NDS-ShopUpdate IsNDS-ShopUpdateAvailable() {
	...
	const char *url;
	if (config->updatenightly()) url = "https://api.github.com/repos/Rinzler65/NDS-Shop/commits";
	else url = "https://api.github.com/repos/Rinzler65/NDS-Shop/releases/latest";
...
std::vector<StoreList> FetchStores() {
	...
	ret = setupContext(hnd, "https://github.com/Rinzler65/NDS-Shop/raw/main/resources/UniStores.json");
	if (ret != 0) {
		socExit();
		free(result_buf);
		free(socubuf);
...
std::string GetChangelog() {
	...
	ret = setupContext(hnd, "https://api.github.com/repos/Rinzler65/NDS-Shop/releases/latest");
	if (ret != 0) {
		socExit();
		free(result_buf);
		free(socubuf);
